...
| `on_full`          | `allow`               | What happens to a new (not-yet-tracked) identity's error response when the zone's shared memory is full: `allow` passes it through untracked (the historical behaviour — a busy zone never blocks traffic); `reject` applies the location's configured rejection status instead, so a full zone fails closed. An **active ban is never evicted** to make room, under either setting — only unblocked, aged-out identities can be reclaimed. |
