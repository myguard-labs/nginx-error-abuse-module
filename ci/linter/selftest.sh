...
# CI.md is a second, workflow-shaped contract. Re-introducing the old
...
# Unit test executables are generated by ci/tests/unit/run.sh and must never
# become addable build output. Remove each rule temporarily to prove the
# control detects the regression, then restore the live file before green.
for unit_bin in ci/tests/unit/test_scan ci/tests/unit/test_time; do
    unit_rule="^${unit_bin//\//\\/}$"
    sed -i "/$unit_rule/d" .gitignore
    case_ 1 "generated $unit_bin is addable without its ignore rule" \
        git check-ignore -q -- "$unit_bin"
    cp "$gitignore_backup" .gitignore
    case_ 0 "generated $unit_bin remains ignored" \
        git check-ignore -q -- "$unit_bin"
done
